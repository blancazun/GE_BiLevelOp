...
def mapping_breadth_first(self, codons, order=None):
    ...
    for order_codon in order:
        ...
        while True:
            ...
            if pro[i] in self.non_terminals:
                pos = codon % len(self.productions[pro[i]])
                #print("".join(pro), f'{codon}%{len(self.productions[pro[i]])}={pos}', order_codon, codon, pos )
                ipos = i
                for item in self.productions[pro[i]][pos]:
                    ipos += 1
                    pro.insert(ipos, item)
...
def mapping_pigrammatical(self, codons, order=None):
    ...
    while i < size and len(nt) > 0:
        ...
        ipos = nt[ntpos] + 1
        #print("".join(pro), f'({nt_value}, {codon})', f'{nt_value}%{len(nt)}={ntpos}', f'{codon}%{len(self.productions[pro[nt[ntpos]]])}={pos}')
...
class Grammar:
    def __init__(self, mapping=mapping_depth_first, file=None, grammar=None):
